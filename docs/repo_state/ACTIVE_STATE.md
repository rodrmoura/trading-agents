...
- Post-P3.4 live structured-output smoke passed on 2026-05-03 through the Extension Development Host gateway at `127.0.0.1:54593` using `claude-sonnet-4.6`. A direct `GatewayChatModel.with_structured_output(TinyDecision, include_raw=True)` call returned `parsed_type=TinyDecision`, `parsing_error=None`, and `raw_tool_calls=1`. The full graph harness then passed for `NVDA` on `2024-05-10` with analyst `market`, processed decision `Hold`, field counts `market_report=7501`, `investment_plan=2551`, `trader_investment_plan=1009`, `final_trade_decision=2523`, and output root `C:\Users\Rodrigo\AppData\Local\Temp\tradingagents-vscode-graph-smoke-p34-20260503004558`. No structured-output unsupported or invocation-fallback warnings appeared in the successful terminal output. The successful graph command required `PYTHONPATH` set to the repository root so the script imports the current workspace provider path.

## Known Blockers

- No current blocker for non-streaming adapter-level structured output. The gateway still does not enforce tool choice or schema adherence.

## Next Actions

- Plan the next `vscode` provider slice after P3.4: gateway-native structured-output contract decisions, model capability advertisement, or tool-enabled streaming.
- For future live graph script runs from the source checkout, set `PYTHONPATH` to the repository root before invoking `scripts/smoke_vscode_tradingagents_graph.py`.
